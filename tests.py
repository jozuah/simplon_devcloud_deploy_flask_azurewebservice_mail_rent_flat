...
class Test_file(unittest.TestCase):
    ...
    def test_mail_sender_mock_fail(self):
        #On va fake la partie sendmail() de smtplib
        with patch("smtplib.SMTP", create=True) as my_stmp:
            my_stmp.login = Exception
            my_stmp.sendmail = Exception
            #my_stmp.login.side_effect = Exception
            #print("test_using_context_manager:", sendmail.call_args_list)
            ...
            self.assertGreater(len(message_return),7)
